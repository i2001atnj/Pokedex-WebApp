    ...
    <>
      ...
      <div className={"sort-menu " + (isOpen ? 'open' : 'closed')}>
        ...
        <div className='sort-menu-section'>
          <label><input type="radio" name='sortBy' value="id" onChange={onIDChange} checked={ state.currentSort === sortTypes.ID ? true : false }/>Number</label>
          <label><input type="radio" name='sortBy' value="name" onChange={onNameChange} checked={ state.currentSort === sortTypes.NAME ? true : false }/>Name</label>
        </div>
      </div>
    </>
  )
